...
const createStory = async (req, res) => {
  try {
    ...
    const result = await db.query(
      "INSERT INTO stories (title, author, content, user_id) VALUES ($1, $2, $3, $4) RETURNING *",
      [title, author, content, user_id]
    );
...
const getStoryById = async (req, res) => {
  try {
    ...
    const result = await db.query(
      "SELECT story_id, title, content, author FROM stories WHERE story_id = $1",
      [storyId]
    );
    if (result.rows.length >= 1) {
      res.status(200).json({ stories: result.rows });
